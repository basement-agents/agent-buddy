...
export function ReviewsPage() {
  ...
  // SSE-based job progress tracking
  useEffect(() => {
    ...
    for (const review of pendingReviews) {
      ...
      // Skip if already connected via SSE
    ...
    // Clean up SSE connections for jobs no longer in the list
    ...
    // Fallback polling when SSE is not supported
    ...
    return () => {
      // Clean up polling interval on re-render or unmount
      if (pollRef.current) {
        clearInterval(pollRef.current);
        pollRef.current = null;
      }
  ...
  if (error) return <ErrorState role="alert" message={`Error: ${error}`} onRetry={() => { updateParam("page", 1); refetch(); }} />;
