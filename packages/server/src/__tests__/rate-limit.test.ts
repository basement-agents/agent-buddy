...
import { rateLimitMiddleware, getRateLimitStatus, reviewRateLimitMiddleware } from "../middleware/rate-limit.js";
...
describe("rateLimitMiddleware", () => {
  ...
  describe("getRateLimitStatus", () => {
    ...
    it("should return static config values", () => {
      const status = getRateLimitStatus();

      expect(status.windowMs).toBe(60000);
      expect(status.publicLimit).toBe(30);
      expect(status.authenticatedLimit).toBe(100);
      expect(typeof status.totalEntries).toBe("number");
    });
  });

  describe("Sliding window behavior", () => {
    it("should reset request count after window expires", async () => {
      const app = new Hono();
      app.use("/*", rateLimitMiddleware());
      app.get("/test", (c) => c.json({ ok: true }));
      ...
      const rateLimitedRes = await app.request("/test", {
        headers: {
          "x-forwarded-for": ip,
        },
      });
      ...
      // Note: We can't easily test the window expiration in a unit test
      // without manipulating Date.now() or waiting 60 seconds
      // This is covered by the sliding window logic in the implementation
    });
  });
});

describe("reviewRateLimitMiddleware", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(loadConfig).mockResolvedValue({
      version: "1.0.0",
  ...
  it("should pass through when under the limit", async () => {
    const app = new Hono();
    app.use("/*", reviewRateLimitMiddleware());
    app.get("/test", (c) => c.json({ ok: true }));
  ...
  it("should return 429 after 10 requests within the window", async () => {
    const app = new Hono();
    app.use("/*", reviewRateLimitMiddleware());
    app.get("/test", (c) => c.json({ ok: true }));
    ...
    for (let i = 0; i < 11; i++) {
      requests.push(
        app.request("/test", { headers: { "x-forwarded-for": ip } })
      );
    }
    ...
    const data = await responses[10].json();
    expect(data).toEqual({
      error: "Rate limit exceeded",
      retryAfter: expect.any(Number),
    });
  ...
  it("should set X-RateLimit-Limit header to 10", async () => {
    const app = new Hono();
    app.use("/*", reviewRateLimitMiddleware());
    app.get("/test", (c) => c.json({ ok: true }));
  ...
  it("should track rate limits independently per client IP", async () => {
    const app = new Hono();
    app.use("/*", reviewRateLimitMiddleware());
    app.get("/test", (c) => c.json({ ok: true }));

    // IP1 uses all 10 slots
    const ip1Requests = [];
    for (let i = 0; i < 10; i++) {
      ip1Requests.push(
        app.request("/test", { headers: { "x-forwarded-for": "10.0.0.5" } })
      );
    }
    await Promise.all(ip1Requests);

    // IP2 should still have 10 available
    const ip2Res = await app.request("/test", {
      headers: { "x-forwarded-for": "10.0.0.6" },
    });
    expect(ip2Res.status).toBe(200);
    expect(ip2Res.headers.get("X-RateLimit-Remaining")).toBe("9");
  });
});
